...
const Directors = () => {
    ...
    const [directorStates, setDirectorStates] = useState(directors.map(() => false));
                    ...
                    <ul className='grid grid-cols-3 gap-y-20 gap-x-30'>
                        {directors.map((e, index) => (
                            <li key={index} className='bg-sacramentoStateGreen pt-8 pb-14 px-6 flex flex-col items-center relative'>
                                <img className='mb-4' width={96} height={96} src={e.img} alt={e.title} />
                                <div className='text-center'>
                                       <h3 className='header-link text-raptureBlue'>{e.name}</h3>
                                    <p className='text-still-base italic font-medium'>{e.job}</p>
                                </div>
                                <button onClick={() => toggleDirectorState(index)} className={`${directorStates[index] ? '-rotate-45' : 'rotate-0'} transition-all w-16 h-16 bg-lightCoral rounded-full absolute flex justify-center items-center -bottom-8 z-10 hover:bg-raptureBlue duration-200`}>
                                    <img className='' src={pilus} alt="pilus icon" />
                                </button>

                                {/* about  */}
                                <div className={`${directorStates[index] ? 'flex' : 'hidden'} absolute  top-0 right-0 left-0 bottom-0 flex-col py-9 px-12 w-full h-full items-center directors-card bg-black`}>
                                    <h3 className=" mb-2">{e.name}</h3>
                                    <p className=" mb-6 ">{e.description}</p>
                                    <ul className="flex items-center space-x-4">
                                        <li>
                                            <a href={e.twitter} target='_blank'>
                                                <svg xmlns="http://www.w3.org/2000/svg" className='text-white hover:text-lightCoral transition-colors' width="20" height="24" viewBox="0 0 24 20" fill="none">
                                        ...
                                        <li>
                                            <a href={e.linkedin} target='_blank'>
                                                <svg className='text-white hover:text-lightCoral  transition-colors' width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <g id="icon-linkedin">
